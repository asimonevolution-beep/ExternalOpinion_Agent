...
const envSchema = z.object({
  ...
  CARICO_ASTE_DIR: z.string().trim().default('CARICO_ASTE'),
  REPORT_FINALI_DIR: z.string().trim().default('REPORT_FINALI'),
  REGISTRO_ORE_FILE: z.string().trim().default('registro_attivita.txt'),
  LOG_FILE: z.string().trim().default('logs/external-opinion.log'),
  LOG_LEVEL: z.enum(['fatal', 'error', 'warn', 'info', 'debug', 'trace']).default('info'),
  PDF_DOWNLOAD_TIMEOUT_MS: z.coerce.number().int().positive().default(20000),
  WHITELIST_HOSTS: z.string().optional().default(''),
...
const whitelistHosts = env.WHITELIST_HOSTS
  .split(',')
...
module.exports = {
  ...
  downloadTimeoutMs: env.PDF_DOWNLOAD_TIMEOUT_MS,
  whitelistHosts,
  cashflow: {
